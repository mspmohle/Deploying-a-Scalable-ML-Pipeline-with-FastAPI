...
# ======== CONFIG ========
# Set your label here. Common values for the "adult/census" task:
#   "salary"   (values like <=50K, >50K)   or
#   "income"   (values like <=50K, >50K)
LABEL = "salary"

# Optional: if you want to explicitly control categorical features, list them here.
# If left as None, we'll auto-detect as columns of dtype 'object' (string-like).
...
def _ensure_binary_labels(y: Iterable) -> np.ndarray:
    """
    Convert string labels like '<=50K' / '>50K' into 0/1 if needed.
    If y is already 0/1 numeric, it is returned unchanged.
    """
    y = np.asarray(y)
    if y.dtype.kind in {"i", "u", "b"}:
        return y.astype(int)

    # Heuristic for adult/census labels. You may adapt as needed.
    mapping = {
    ...
    # If anything is non-numeric here, raise to force user to clarify.
    try:
        return y_bin.astype(int)
    except Exception as exc:
        raise ValueError(
            "Could not coerce labels to 0/1. Update LABEL/mapping in app/model.py."
        ) from exc


def infer_categorical_features(df: pd.DataFrame) -> List[str]:
    """Infer categorical columns as object/string dtype when not explicitly provided."""
...
def split_features(
    ...
    """Split X/y and return features separated but not yet encoded."""
    X = df.copy()
    y = None
    ...
    # Ensure all categoricals are string to avoid encoder surprises
...
def process_data(
    ...
    """
    Convert a raw DataFrame into a numeric matrix suitable for model training/inference.
    Returns (X_processed, y, encoder).
    """
    X_all, y = split_features(X, label=label, categorical_features=categorical_features)

    # Recompute num/cat after the split to preserve column order
...
def load_artifacts(art: Artifacts = Artifacts()) -> Tuple[RandomForestClassifier, OneHotEncoder]:
